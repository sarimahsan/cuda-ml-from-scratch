"""
=============================================================================
CUDA ML BENCHMARK SUITE: BASIC ELMAN RECURRENT NEURAL NETWORK (RNN)
=============================================================================
Comprehensive correctness verification and micro/macro benchmarking:
  1. Forward & Backward (BPTT) Numerical Gradient Checking vs torch.nn.RNN
  2. Microbenchmark: Step & Sequence Operations vs Native PyTorch
...
try:
    from rnn import CUDARNN, CUDARNNCell, _ext
except ImportError as e:
    print(f"[ERROR] Failed to import CUDA RNN: {e}")
    sys.exit(1)
...
def time_cuda_operation(func: Callable, warmup: int = 25, reps: int = 100) -> Tuple[float, float]:
    ...
    std_ms = 0.0
    return elapsed_ms, std_ms


def test_correctness(device: str = "cuda"):
    print("\n" + "=" * 85)
    print(" 1. NUMERICAL CORRECTNESS & GRADIENT VERIFICATION (vs torch.nn.RNN)")
    print("=" * 85)

    T, N, D, H = 16, 8, 32, 64
    torch.manual_seed(42)

    # 1. Instantiate PyTorch Native and Custom CUDA RNN
    rnn_torch = nn.RNN(input_size=D, hidden_size=H, nonlinearity="tanh", bias=True, batch_first=False).to(device)
    rnn_cuda = CUDARNN(input_size=D, hidden_size=H, nonlinearity="tanh", bias=True).to(device)

    # Sync weights: PyTorch stores weight_ih as [H, D] and weight_hh as [H, H]
    with torch.no_grad():
        rnn_cuda.weight_ih.copy_(rnn_torch.weight_ih_l0.t())
        rnn_cuda.weight_hh.copy_(rnn_torch.weight_hh_l0.t())
        rnn_cuda.bias_ih.copy_(rnn_torch.bias_ih_l0)
        rnn_cuda.bias_hh.copy_(rnn_torch.bias_hh_l0)
    ...
    # Forward
    out_torch, hn_torch = rnn_torch(X_torch, h0_torch)
    out_cuda, hn_cuda = rnn_cuda(X_cuda, h0_cuda)
    ...
    max_dw_ih_diff = (rnn_torch.weight_ih_l0.grad - rnn_cuda.weight_ih.grad.t()).abs().max().item()
    max_dw_hh_diff = (rnn_torch.weight_hh_l0.grad - rnn_cuda.weight_hh.grad.t()).abs().max().item()
    max_db_ih_diff = (rnn_torch.bias_ih_l0.grad - rnn_cuda.bias_ih.grad).abs().max().item()
...
def benchmark_micro(device: str = "cuda"):
    print("\n" + "=" * 85)
    print(" 2. KERNEL-LEVEL MICROBENCHMARKS: Custom CUDA RNN vs PyTorch Native")
    ...
    rnn_torch = nn.RNN(D, H, nonlinearity="tanh", bias=True, batch_first=False).to(device)
    rnn_cuda = CUDARNN(D, H, nonlinearity="tanh", bias=True).to(device)

    # Forward
    t_fwd_cuda, _ = time_cuda_operation(lambda: rnn_cuda(X, h0))
    t_fwd_torch, _ = time_cuda_operation(lambda: rnn_torch(X, h0_torch))
    sp_fwd = t_fwd_torch / max(t_fwd_cuda, 1e-6)
    print(f"{'RNN Sequence Forward (T=64, N=64)':<42} | {t_fwd_cuda:>9.4f} ms | {t_fwd_torch:>10.4f} ms | {sp_fwd:>6.2f}x")

    # Forward + Backward
    def run_cuda_full():
        x_c = X.clone().detach().requires_grad_(True)
        out, _ = rnn_cuda(x_c, h0)
        out.sum().backward()

    def run_torch_full():
        x_t = X.clone().detach().requires_grad_(True)
        out, _ = rnn_torch(x_t, h0_torch)
        out.sum().backward()
    ...
    print(f"{'RNN Full Forward + BPTT (T=64, N=64)':<42} | {t_full_cuda:>9.4f} ms | {t_full_torch:>10.4f} ms | {sp_full:>6.2f}x")
    print("-" * 85)
...
def main():
    parser = argparse.ArgumentParser(description="CUDA ML Benchmark Suite: Basic RNN")
    ...
    print("=" * 85)
    print(" 🚀 CUDA ML BENCHMARK SUITE: BASIC RECURRENT NEURAL NETWORK (RNN) 🚀")
