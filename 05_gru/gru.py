"""
=============================================================================
Modular Pure CUDA Gated Recurrent Unit (GRU): Python Interface
=============================================================================
Provides high-level PyTorch-compatible abstractions:
  - CUDAGRUCell: Single timestep recurrent cell
  - CUDAGRU: Multi-timestep sequence model with full BPTT autograd
  - CUDAGRULanguageModel: Character/token language model with text generation
=============================================================================
"""
...
sources = [
    ...
    os.path.join(current_dir, "csrc", "gru_cell.cu"),
...
try:
    import cuda_gru as _ext  # type: ignore
except ImportError:
    print("[INFO] Compiling modular CUDA GRU extension via JIT using centralized kernels...")
    _ext = load(
        name="cuda_gru",
        sources=sources,
        extra_include_paths=[
    ...
    print("[INFO] CUDA GRU JIT compilation successful!")


# -----------------------------------------------------------------------------
# 1. Custom Autograd Function for High-Performance Sequence GRU
# -----------------------------------------------------------------------------
class _CUDAGRUSequenceFunction(torch.autograd.Function):
    @staticmethod
    def forward(ctx, X_seq, W_ih, b_ih, W_hh, b_hh, h_0):
        H_seq, h_T, gates_act_seq, G_hh_seq, _ = _ext.gru_forward_sequence(
            X_seq.contiguous(),
            W_ih.contiguous(),
        ...
        ctx.save_for_backward(X_seq, W_ih, b_hh, W_hh, h_0, H_seq, gates_act_seq, G_hh_seq)
        return H_seq, h_T

    @staticmethod
    def backward(ctx, dH_seq, dh_T):
        X_seq, W_ih, b_hh, W_hh, h_0, H_seq, gates_act_seq, G_hh_seq = ctx.saved_tensors

        dH_seq_total = dH_seq.clone() if dH_seq is not None else torch.zeros_like(H_seq)
        if dh_T is not None:
            dH_seq_total[-1] += dh_T

        dW_ih, db_ih, dW_hh, db_hh, dX_seq = _ext.gru_backward_sequence(
        ...
        return dX_seq, dW_ih, db_ih_ret, dW_hh, db_hh_ret, None
...
class CUDAGRUCell(nn.Module):
    """
    Single-step Gated Recurrent Unit (GRU) Cell matching torch.nn.GRUCell:
    ...
    def __init__(self, input_size: int, hidden_size: int, bias: bool = True):
        ...
        # Parameter weights [D, 3H] and [H, 3H]
    ...
    def forward(self, x: torch.Tensor, h_prev: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        h_out, _, _ = _ext.gru_cell_forward(x, h_prev, self.weight_ih, b_ih, self.weight_hh, b_hh)
        return h_out


class CUDAGRU(nn.Module):
    """
    Multi-timestep Gated Recurrent Unit (GRU) Sequence Model with hardware BPTT.
    """
    def __init__(self, input_size: int, hidden_size: int, bias: bool = True):
        super().__init__()
        self.input_size = input_size
    ...
    def forward(self, x_seq: torch.Tensor, h_0: Optional[torch.Tensor] = None) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        return _CUDAGRUSequenceFunction.apply(
            x_seq, self.weight_ih, self.bias_ih, self.weight_hh, self.bias_hh, h_0
        )


class CUDAGRULanguageModel(nn.Module):
    """
    Complete Character/Token Language Model using pure CUDA GRU.
    """
    def __init__(self, vocab_size: int, embed_dim: int, hidden_size: int):
        super().__init__()
        self.vocab_size = vocab_size
        self.embed = nn.Embedding(vocab_size, embed_dim)
        self.gru = CUDAGRU(embed_dim, hidden_size)
    ...
    def forward(self, tokens: torch.Tensor, h_0: Optional[torch.Tensor] = None) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        H_seq, h_T = self.gru(embeddings, h_0) # [T, N, H]
    ...
    def generate(self, seed_indices: list, max_length: int = 100, temperature: float = 0.8) -> list:
        ...
        for _ in range(max_length):
            emb = self.embed(cur_token)
            H_seq, h = self.gru(emb, h)
            logits = self.head(H_seq.squeeze(0)) / max(temperature, 1e-5)
